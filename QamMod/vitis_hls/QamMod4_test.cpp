...
int main() {
  ...
  for (int i = 0; i < std::pow(2, NUMBITSPERSYMB); i++) {
    QamMod4(BITS_T(i), constellation);
    real.push_back(constellation.real().to_double());
    imag.push_back(constellation.imag().to_double());
    binary.push_back(double(i));
  }
  utils::matwrite_double("QamMod4_real.bin", real);
  utils::matwrite_double("QamMod4_imag.bin", imag);
  utils::matwrite_double("QamMod4_binary.bin", binary);
